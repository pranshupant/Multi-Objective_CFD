...
class airfoil:
    ...
    def cfd(self):  #Extract result from post processing
        ...
        if os.path.isfile('postProcessing/forceCoeffs1/0/forceCoeffs.dat'):
            ...
            p = re.findall(r'3000\s+(-?\d\.\d{6})e([-+\d]{3})\s+(-?\d\.\d{6})e([-+\d]{3})\s+(-?\d\.\d{6})e([-+\d]{3})', line)
        ...
        R=[]
        R.append(self.cost)
        np.savetxt('../../cost-%d'%self.__specie, R)

        for i in range(nProc):
            shutil.rmtree('processor%d'%i)

        shutil.rmtree('dynamicCode')
        shutil.rmtree('dynamicCode.orig')

        os.chdir('../../../../../optimisation_code')
        shutil.rmtree('Results_CFD/Generation_%i/Specie_%i/CFD'%(self.__generation,self.__specie))
    ...
    def show(self, g, s):   #display using matplotlib
        ...
        plt.savefig(cfd_image%(g, s, g, s), bbox_inches = 'tight')
    ...
    def savefig(self):   #display using matplotlib
        ...
        plt.savefig(cfd_image%(self.__generation,self.__specie,self.__generation,self.__specie), bbox_inches = 'tight')
        plt.close()
